...
/**
 * Provides convenience methods for in-app billing. You can create one instance of this class for your application and use it to process in-app billing operations. It provides synchronous (blocking) and asynchronous (non-blocking) methods
 * for many common in-app billing operations, as well as automatic signature verification.
 * 
 * After instantiating, you must perform setup in order to start using the object. To perform setup, call the {@link #startSetup} method and provide a listener; that listener will be notified when setup is complete, after which (and not
 * before) you may call other methods.
 * 
 ...
 * 
 * When you are done with this object, don't forget to call {@link #dispose} to ensure proper cleanup. This object holds a binding to the in-app billing service, which will leak unless you dispose of it correctly. If you created the object
 * on an Activity's onCreate method, then the recommended place to dispose of it is the Activity's onDestroy method.
 * 
 * A note about threading: When using this object from a background thread, you may call the blocking versions of methods; when using from a UI thread, call only the asynchronous versions and handle the results via callbacks. Also, notice
 * that you can only call one asynchronous operation at a time; attempting to start a second asynchronous operation while the first one has not yet completed will result in an exception being thrown.
 * 
...
public class IabHelper {
	...
	/**
	 * Creates an instance. After creation, it will not yet be ready to use. You must perform setup by calling {@link #startSetup} and wait for setup to complete. This constructor does not block and is safe to call from a UI thread.
	 * 
	 * @param ctx
	 *            Your application or Activity context. Needed to bind to the in-app billing service.
	 * @param base64PublicKey
	 *            Your application's public key, encoded in base64. This is used for verification of purchase signatures. You can find your app's base64-encoded public key in your application's page on Google Play Developer Console. Note
	 *            that this is NOT your "developer public key".
	 */
	...
	/**
	 * Dispose of object, releasing resources. It's very important to call this method when you are done with this object. It will release any resources used by it such as service connections. Naturally, once the object is disposed of, it
	 * can't be used again.
	 */
	public void dispose() {
		logDebug("Disposing.");
	...
	public interface OnIabPurchaseFinishedListener {
		/**
		 * Called to notify that an in-app purchase finished. If the purchase was successful, then the sku parameter specifies which item was purchased. If the purchase failed, the sku and extraData parameters may or may not be null,
		 * depending on how far the purchase process went.
		 * 
		 * @param result
	...
	/**
	 * Initiate the UI flow for an in-app purchase. Call this method to initiate an in-app purchase, which will involve bringing up the Google Play screen. The calling activity will be paused while the user interacts with Google Play, and
	 * the result will be delivered via the activity's {@link android.app.Activity#onActivityResult} method, at which point you must call this object's {@link #handleActivityResult} method to continue the purchase flow. This method MUST be
	 ...
	 * @param extraData
	 *            Extra data (developer payload), which will be returned with the purchase data when the purchase completes. This extra data will be permanently bound to that purchase and will always be returned when the purchase is
	 *            queried.
	 */
	...
	/**
	 * Handles an activity result that's part of the purchase flow in in-app billing. If you are calling {@link #launchPurchaseFlow}, then you must call this method from your Activity's {@link android.app.Activity@onActivityResult} method.
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		...
		System.out.println("----------------");
		System.out.println(" Billing request code: " + requestCode);
		System.out.println(" Billing result code: " + resultCode);
		System.out.println(" Billing response code: " + responseCode);
...
//				logDebug("Purchase signature successfully verified.");
				...
				logError("Failed to parse purchase data: " + e.getClass().getSimpleName() + " " + e.getMessage());
//				e.printStackTrace();
...
//				return true;
			...
			result = new IabResult(responseCode, "Purchase canceled: " + getResponseDesc(responseCode));
			if (mPurchaseListener != null)
				mPurchaseListener.onIabPurchaseFinished(result, null);
		} else {
	...
	/**
	 * Queries the inventory. This will query all owned items from the server, as well as information on additional skus, if specified. This method may block or take long to execute. Do not call from a UI thread. For that, use the
	 * non-blocking version {@link #refreshInventoryAsync}.
	 * 
	...
	/**
	 * Asynchronous wrapper for inventory query. This will perform an inventory query as described in {@link #queryInventory}, but will do so asynchronously and call back the specified listener upon completion. This method is safe to call
	 * from a UI thread.
	 * 
	 * @param querySkuDetails
	...
	/**
	 * Consumes a given in-app product. Consuming can only be done on an item that's owned, and as a result of consumption, the user will no longer own it. This method may block or take long to return. Do not call from the UI thread. For
	 * that, see {@link #consumeAsync}.
	 * 
	 * @param itemInfo
	...
	public static String getResponseDesc(int code) {
		...
		String[] iabhelper_msgs = ("0:OK/-1001:Remote exception during initialization/" + "-1002:Bad response received/" + "-1003:Purchase signature verification failed/" + "-1004:Send intent failed/" + "-1005:User cancelled/"
				+ "-1006:Unknown purchase response/" + "-1007:Missing token/" + "-1008:Unknown error/" + "-1009:Subscriptions not available/" + "-1010:Invalid consumption attempt").split("/");
